//
//  FahrenheitToKelvinViewController.swift
//  Temperature
//
...
class FahrenheitToKelvinViewController: UIViewController, UIPickerViewDelegate, SideBarDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = "Fahrenheit to Kelvin"
    ...
    func sideBarDidSelectButtonAtIndex(_ index: Int) {
        if index == 0 {
            ...
            let storyboard = UIStoryboard(name: "Main", bundle: nil)
            let ivc = storyboard.instantiateViewController(withIdentifier: identity7)
            ivc.modalPresentationStyle = .custom
            ivc.modalTransitionStyle = .crossDissolve
            self.navigationController?.pushViewController(ivc, animated: true)
    ...
    @IBOutlet var tempRange: TemperatureRange4!
    @IBOutlet weak var tempPicker: UIPickerView!
    @IBOutlet weak var tempLabel: UILabel!

    fileprivate let converter = fToKUnitConverter()
    
    func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
        let fahrenheitValue = tempRange.values[row]
    ...
    func displayConvertedTemperatureForRow(_ row: Int){
        let degreesFahrenheit = tempRange.values[row]
        
        tempLabel.text = "\(converter.degreesKelvin(degreesFahrenheit)) K"
    }
    /*
    // MARK: - Navigation
